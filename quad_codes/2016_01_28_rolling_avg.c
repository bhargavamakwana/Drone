...
int  times=0, gyrosend=0;
long  a_uart=0, gyrodata=0;
...
float GyroBuffer[3] = {0.0f},Gyrooffset[3]={0.0f},Gyrooffset_prev[3]={0.0f},Gyroavg[3],Gyroavg_1[3]={0.0f},Gyroavg_2[3]={0.0f},Gyroangle[3] = {0.0f},Accavg[3]={0.0f},Accavg_1[3]={0.0f},t=1.0f/40.0f,Final_gyro[3],observe[100];
		...
		if(n>=500)
			{
				...
				for(int i=0;i<=4;i++)
				{
					if(i!=4)
					{
						for(int j=0;j<=2;j++)
						{
							arr_gyro[i][j]=arr_gyro[i+1][j];
							arr_acc[i][j]=arr_acc[i+1][j];
							arr_mag[i][j]=arr_mag[i+1][j];
						}
					}
					if(i==4)
					{
						for(int j=0;j<=2;j++)
						{
							arr_gyro[i][j]=GyroBuffer[j];//-Gyrooffset[j];
							arr_acc[i][j]=AccBuffer[j];
							arr_mag[i][j]=MagBuffer[j];
						}
					}
				}
			...
			for(int i=0;i<=2;i++)
			{
				for(int j=0;j<=4;j++)
				{
					Gyroavg[i]+=((j+1)*arr_gyro[j][i]);
					Accavg[i]+=((j+1)*arr_acc[j][i]);
					Magavg[i]+=((j+1)*arr_mag[j][i]);
				}
			}
...
void gyroConfig(void)
{
  ...
  L3GD20_InitStructure.Output_DataRate = L3GD20_OUTPUT_DATARATE_2;//1-95 ,2-190,, 4-760
  L3GD20_InitStructure.Axes_Enable = L3GD20_AXES_ENABLE;
  L3GD20_InitStructure.Band_Width = L3GD20_BANDWIDTH_3;    //3-frequency above 50Hz will be rejected
