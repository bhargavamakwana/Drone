...
float pitch=0.0f,pitch_acc=0.0f,set_pitch[2]={0.0f},error_pitch[2]={0.0f},sum_pitch=0.0f;
float roll=0.0f,roll_acc=0.0f,set_roll[2]={0.0f},error_roll[2]={0.0f},sum_roll=0.0f;
...
float c7=0.0f,c8=0.0f,a4=0.0f;

//pitch//
float e9=0.0f,ddset_pitch=0.0f,e10=0.0f;
float c9=.9f,c10=0.5f,a5=.29f;
/*********************************/
...
int flag=0, pidupdate=0,bstepping=0;
...
float pitch_est[1]={0.0f},pitch_inn=0.0f,pitch_rate=0.0f,pitch_ang_vel=0.0f,pitch_ang_vel_inn=0.0f,pitch_den=0.0f;
...
float roll_est[1]={0.0f},roll_inn=0.0f,roll_rate=0.0f,roll_ang_vel=0.0f,roll_ang_vel_inn=0.0f,roll_den=0.0f;
...
//				}		
						...
						gyrodata = (pitch_est[1]+1000)*1000;
								...
								//{
									//bstepping=0;
																										...
																										/**************************************backstepping controller****************************************/
																											...
																											/**attitude control**/
																											e7=roll_est[1]-set_roll[2];
																																//double derivative of set_roll//
																											ddset_roll=(set_roll[2]-2*set_roll[1]+set_roll[0])/t*t;
																											e8= c7*e7+a4*(sum_roll+e7*t)+(set_roll[2]-set_roll[1])/t-(roll_est[1]-roll_est[0])/t;
																											ccr_roll= (1-c7*c7+a4)*e7+(c7+c8)*e8-c7*a4*sum_roll+ddset_roll;
																											roll_est[0]=roll_est[1];
																											set_roll[0]=set_roll[1];
																											set_roll[1]=set_roll[2];
																																
																																
																												e9=pitch_est[1]-set_pitch[2];
																																//double derivative of set_pitch//
																											ddset_roll=(set_pitch[2]-2*set_pitch[1]+set_pitch[0])/t*t;
																											e10= c9*e9+a5*(sum_pitch+e7*t)+(set_pitch[2]-set_pitch[1])/t-(pitch_est[1]-pitch_est[0])/t;
																											ccr_pitch= (1-c9*c9+a5)*e9+(c9+c10)*e9-c9*a5*sum_pitch+ddset_pitch;
																											pitch_est[0]=pitch_est[1];
																											set_pitch[0]=set_pitch[1];
																											set_pitch[1]=set_pitch[2];				
																										static int i1=1;
																										if(i1==1)
																										{
																												GPIOA->BSRR = 0x0002;
...
//state prediction
																						...
																						pitch_est[1]=pitch_est[1]+(t_kal*pitch_rate);        
																						...
																						//State innovation
																						pitch_inn=pitch_acc-pitch_est[1];
																						...
																						//state update
																						pitch_est[1] += (pitch_K[0][0] * pitch_inn) + (pitch_K[0][1] * pitch_ang_vel_inn);
																						...
																						roll_est[1]=roll_est[1]+(t_kal*roll_rate);
																						...
																						//state update
																						roll_est[1] += (roll_K[0][0] * roll_inn) + (roll_K[0][1] * roll_ang_vel_inn);
...
//timer 3 pwm outputs running at 50Hz
	PrescalerValue=(uint16_t)(SystemCoreClock/500000)-1;
