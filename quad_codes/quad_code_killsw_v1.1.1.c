...
#define angle                         (float)     57.295779578552f
...
int throt_check=0,integral_flag=0;
...
float u=0.0f,u_roll=0.0f,u_pitch=0.0f,u_yaw=0.0f,kp=0.0f,ki=0.0f,kd=0.0f,ccr;       //pitch 1.3,1.9,0.018

float k_p[3]={1.4,1.5,2.4},k_i[3]={0.135,0.115,0.0},k_d[3]={0.24,0.29,0.2};        //stable values pitch 0.56,0.115,0.29
float tmp_roll,tmp_pitch=0;
float sin_angle,cos_angle,angle_raw=0;
//float k_p[3]={0.0,0.0,0},k_i[3]={0.0,0.0,0.0},k_d[3]={0.0,0.0,0},p_roll=0.0f;//0.4444=k_p k_d=0.0277 k_d[0]=0.01944 2.1,0.11,0.25
...
//float k_p[3]={0.0,1.3,0.0},k_i[3]={0.0,1.85,0.0},k_d[3]={0.0,0.018,0.0};
...
float GyroBuffer[3] = {0.0f},Gyrooffset[3]={0.0f},Gyrooffset_prev[3]={0.0f},Gyroavg[3],Gyroavg_1[3]={0.0f},Gyroavg_2[3]={0.0f},Gyroangle[3] = {0.0f},Accavg[3]={0.0f},Accavg_1[3]={0.0f},t_kal=1.0f/180.0f,t=1.0f/100.0f,Final_gyro[3],observe[100];
...
char m=1;
float MagBuffer[3] = {0.0f},mag=0.0f,dir=0.0f,initdir=0.0f,yaw_dir=0.0f,sum_dir=0.0f;
unsigned int mag_angle=0;
...
float R_angle_pitch = 100000.0f;       //1000
float R_ang_vel_pitch = 1.0f;     //0.001
float pitch_est1=0.0f,pitch_est=0.0f,pitch_inn=0.0f,pitch_rate=0.0f,pitch_ang_vel=0.0f,pitch_ang_vel_inn=0.0f,pitch_den=0.0f;
...
double Q_angle_roll = 0.0001f;    //0.001
//Q_gyroBias should be low, since gyro bias change extremely slow
double Q_ang_vel_roll = 0.001f;
double Q_gyroBias_roll = 0.01f;     //0.003
//R_measure should be greater than Q_angle to prevent acceleration from affecting the angle
double R_angle_roll = 100000.0f;       //0.03
double R_ang_vel_roll = 1.0f;
float roll_est1=0.0f,roll_est=0.0f,roll_inn=0.0f,roll_rate=0.0f,roll_ang_vel=0.0f,roll_ang_vel_inn=0.0f,roll_den=0.0f;
...
unsigned int edge1=0,edge2=0,edge3=0,edge4=0,edge5=0,edge6=0,edge7=0,edge8=0,frequency=0,CCR=0,init=54,final=35949,CCRshow=0,flag1=0;
float Capture[2]={0.0f},CCR_3=0.0f,CCR_2=0.0f,gap1=0.0f;
int counts1=0,start1=0,check1=0,counter1=0;
int counts2=0,check2=0;

int counts3=0,check3=0,counter3=0;
float CCR_1=0.0f;
unsigned int kill_sw=0;
...
unsigned int gyrosend,var=0,i=0;	
		...
		if(takeaction==1)
		{
		  ...
		  TIM3->CCR1=CCR1_Val;     //front
 			...
 			if(pidupdate==1)
			{
				...
				if(ccr_throttle>20 && offset==100)
				{
				
//				
//				roll_est = (int) (roll_est*10);								
//				roll_est = (float)(roll_est/10);
//				pitch_est = (int) (pitch_est*10);								
//				pitch_est = (float)(pitch_est/10);					
////				 	
				
				error_roll[0]=error_roll[1];
				error_roll[1]=roll_est-set_roll-roll_est1;
				...
				//ccr_roll=0;
				//p_roll=k_p[0]*error_roll[1];
				//ccr_roll=(250*u_roll)/7.7;
				
				error_pitch[0]=error_pitch[1];
				error_pitch[1]=pitch_est-set_pitch-pitch_est1;
...
// 				
					...
					//	meas[1]=-1.0*Gyroangle[1]*0.98 + roll_acc*0.02;
						...
						//yaw_dir = angle * acos(cos((90-dir)/angle) * cos(initdir/angle) + sin((90-dir)/angle)*sin(initdir/angle));
					//	yaw_dir = angle * asin(-sin((dir)/angle) * cos(initdir/angle) + cos((dir)/angle)*sin(initdir/angle));
							sin_angle =(-sin((dir)/angle) * cos(initdir/angle) + cos((dir)/angle)*sin(initdir/angle));
							cos_angle =(cos((dir)/angle) * cos(initdir/angle) + sin((dir)/angle)*sin(initdir/angle));
							yaw_dir=angle*acos(cos_angle);
...
//timer 3 pwm outputs
  ...
  NVIC_Init(&NVIC_InitStructure);
	...
	PrescalerValue=(uint16_t)(SystemCoreClock/3500)-1;
	TIM_TimeBaseStructure.TIM_Period = 40000;						//40000
  TIM_TimeBaseStructure.TIM_Prescaler = 18;						//18
  TIM_TimeBaseStructure.TIM_ClockDivision = 0;
  TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
...
/* TIM2 channel 2 pin (Pd.04) configuration */
  GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_4 | GPIO_Pin_6 | GPIO_Pin_7|GPIO_Pin_3;
	...
	if(!(TIM_GetITStatus(TIM2, TIM_IT_CC3)) == RESET) 
  {
		...
		if(check1==0)
    ...
    else if(check1==1) 
			{
				...
				//k_i[0]=0.5+CCR_3;
				set_roll=(-CCR_3);		
				set_roll = (int) (set_roll*10);
				set_roll = (float)(set_roll/10);	
//k_p[1]=0.56+(-CCR_3	)/5;
//				k_p[1] = (int) (k_p[1]*1000);
//				k_p[1] = (float)(k_p[1]/1000);				
			//	k_i[2]=  (-CCR_3)/40;
				//k_i[2] = (int) (k_i[2]*1000);
				//k_i[2] = (float)(k_i[2]/1000);
	...
	if(!(TIM_GetITStatus(TIM2, TIM_IT_CC2)) == RESET) 
  {
		...
		if(check2==0)
    ...
    else if(check2==1) 
			{
				...
				set_pitch=CCR_2;
				set_pitch = (int) (set_pitch*10);
				set_pitch = (float)(set_pitch/10);
	...
	if(!(TIM_GetITStatus(TIM2, TIM_IT_CC1)) == RESET) 
  {
		...
		if(check3==0)
    ...
    else if(check3==1) 
			{
				edge8 = TIM2->CCR1;
				check3=0;
				counts3=36000*counter3 + edge8-edge7;	
				if (counts3>36000)
						counts3=18000;
				CCR_1=(float)(counts3-1350)*100.0/2700.0;//5 degrees till 1600
...
//		static int tim7check =0, tim7flag=0;
//		tim7check++;
//		if(tim7check==100)
//		{tim7flag++;tim7check=0;}
