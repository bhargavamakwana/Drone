...
float pitch=0.0f,pitch_acc=0.0f,error_pitch[2]={0.0f},sum_pitch=0.0f,set_pitch[3]={0.0f};
unsigned long pitch_est[2]={0.0};
float roll=0.0f,roll_acc=0.0f,error_roll[2]={0.0f},sum_roll=0.0f,set_roll[3]={0.0f};
unsigned long roll_est[2]={0.0},yaw_est[2]={0.0};
float yaw=0.0f,yaw_acc=0.0f,set_yaw[3]={0.0f},error_yaw[2]={0.0f},sum_yaw=0.0f;
...
/*********backstepping initialisations*************/
//roll//
float roll_est_latest=0.0f,roll_est_prev=0.0f,set_roll_latest=0.0f,set_roll_prev1=0.0f,set_roll_prev0=0.0f;
float e7=0.0f,ddset_roll=0.0f,e8=0.0f;
float c7=0.0f,c8=0.0f,a4=0.0f;				//c8 is between 0.5 to 0.05 a4 can be very vaugely between 0-100
																				// 0.4,0.05,-75.0 last values of roll constants
//pitch//
float pitch_est_latest=0.0f,pitch_est_prev=0.0f,set_pitch_latest=0.0f,set_pitch_prev1=0.0f,set_pitch_prev0=0.0f;
float e9=0.0f,ddset_pitch=0.0f,e10=0.0f;
float c9=0.0f,c10=0.0f,a5=0.0f;

//yaw//
float yaw_est_latest=0.0f,yaw_est_prev=0.0f;
float e11=0.0f,e12=0.0f,ddset_yaw=0.0f;
float c11=0.0f,c12=0.0f,a6=0.0f;			//c11 should be less than 2.4 
/*********************************/
...
float GyroBuffer[3] = {0.0f},Gyrooffset[3]={0.0f},Gyrooffset_prev[3]={0.0f},Gyroavg[3],Gyroavg_1[3]={0.0f},Gyroavg_2[3]={0.0f},Gyroangle[3] = {0.0f},Accavg[3]={0.0f},Accavg_1[3]={0.0f},t=1.0f/180.0f,Final_gyro[3],observe[100];
...
float Capture[2]={0.0f},CCR_3=0.0f,CCR_2=0.0f,gap1=0.0f,wx=0.0f;
...
//	RCC_SYSCLKConfig (RCC_SYSCLKSource_PLLCLK);
   ...
   while (1)
  {
		...
		//	uint8_t sysclock=RCC_GetSYSCLKSource ();
...
// 				
						...
						pitch_est_latest=-1.0*Gyroangle[0]*0.98 + pitch_acc*0.02;
						roll_est_latest=-1.0*Gyroangle[1]*0.98 + roll_acc*0.02;
						...
						a=(mag_Y*cos(roll_est[1]/kangle));
						b=(mag_Z*sin(roll_est[1]/kangle));
						Yh=a-b;
						Xh=mag_X*cos(pitch_est[1]/kangle)+mag_Y*sin(roll_est[1]/kangle)*sin(pitch_est[1]/kangle)+mag_Z*cos(roll_est[1]/kangle)*sin(pitch_est[1]/kangle);
						...
						if(Xh>0&&Yh<0)
 						...
 						if(m==1)
 							initdir=dir;
								...
								m=2;
								temp1=dir-initdir;
								yaw_est[1]=-1.0*Gyroangle[2]*0.98+(dir-initdir)*0.02;

						
					
				/*backstepping control*/
																											
//																											e7=set_roll_latest-roll_est_latest;
//																																//double derivative of set_roll//
//																											sum_roll=sum_roll+e7*t;
//																											ddset_roll=(set_roll_latest-2*set_roll_prev1+set_roll_prev0)/t*t;
//																											e8= c7*e7+a4*(sum_roll)+(set_roll_latest-set_roll_prev1)/t-(roll_est_latest-roll_est_prev)/t;
//																											ccr_roll= (1-c7*c7+a4)*e7+(c7+c8)*e8-c7*a4*sum_roll+ddset_roll;
//																											//roll_est[0]=roll_est[1];
//																											roll_est_prev=roll_est_latest;
//																											set_roll_prev0=set_roll_prev1;					
//																											set_roll_prev1=set_roll_latest;
//																											
																											e9=set_pitch_latest-pitch_est_latest;
																																//double derivative of set_pitch//
																											sum_pitch=sum_pitch+e9*t;					
																											ddset_pitch=(set_pitch_latest-2*set_pitch_prev1+set_pitch_prev0)/t*t;
																											wx=(pitch_est_latest-pitch_est_prev)/t;
																											e10= c9*e9+a5*(sum_pitch)+(set_pitch_latest-set_pitch_prev1)/t-(pitch_est_latest-pitch_est_prev)/t;
																											ccr_pitch= (1-c9*c9+a5)*e9+(c9+c10)*e10-c9*a5*sum_pitch+ddset_pitch;
																											//pitch_est[0]=pitch_est[1];
																											pitch_est_prev=pitch_est_latest;
																											set_pitch_prev0=set_pitch_prev1;
																											set_pitch_prev1=set_pitch_latest;
																											
//																											e11=set_yaw[2]-yaw_est_latest;
//																																//double derivative of set_yaw//
//																											sum_yaw=sum_yaw+e11*t;
//																											ddset_yaw=(set_yaw[2]-2*set_yaw[1]+set_yaw[0])/t*t;
//																											e12= c11*e11+a6*(sum_yaw)+(set_yaw[2]-set_yaw[1])/t-(yaw_est_latest-yaw_est_prev)/t;
//																											ccr_yaw= (1-c11*c11+a6)*e9+(c11+c12)*e11-c11*a6*sum_yaw+ddset_yaw;
//																											yaw_est_prev=yaw_est_latest;
//																											set_yaw[0]=set_yaw[1];
//																											set_yaw[1]=set_yaw[2];
//					
...
//				//ccr_yaw=(250*u_yaw)/7.7;
				...
				ccr_left_motor=0;//ccr_throttle+ccr_roll+ccr_yaw;
					if(ccr_left_motor>850)
						ccr_left_motor=850;
					if(ccr_left_motor<0)
						ccr_left_motor=20;
								
				ccr_right_motor=0;//ccr_throttle-ccr_roll+ccr_yaw;
				...
				else
				{
					...
					ccr_left_motor=0;//ccr_throttle;
					ccr_right_motor=0;//ccr_throttle;
				}
 ...
 //while loop end after this 2 brackets  
}
	}
	...
	if(!(TIM_GetITStatus(TIM2, TIM_IT_CC3)) == RESET) 
  {
		...
		if(check1==0)
    ...
    else if(check1==1) 
			{
				...
				if (counts1>36000)
						counts1=18000;
				CCR_3=(float)(counts1-1350)*5.0/2700.0;//5 degrees till 1600
				//k_i[0]=0.5+CCR_3;
				//set_roll_latest=-1*CCR_3;
				c9=-1*CCR_3;
				//set_pitch=CCR_3;
				counter1=0;
			}
	...
	if(!(TIM_GetITStatus(TIM2, TIM_IT_CC2)) == RESET) 
  {
		...
		if(check2==0)
    ...
    else if(check2==1) 
			{
				...
				if (counts2>36000)
						counts2=18000;
				CCR_2=(float)(counts2-1350)*1.0/2700.0;//5 degrees till 1600
				set_pitch_latest=0;//CCR_2;
				counter2=0;
			}
	}
